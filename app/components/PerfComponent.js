...
export default class PerfComponent {
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      if (categoryTracker === 'RENDER') {
        template.renderMetrics.forEach(metric => {
          ...
          emptyRenderTemplate[metric].colorTheme =
            '#' + Math.floor(Math.random() * 16777215).toString(16);
        });
      } else if (categoryTracker === 'SUMMARY') {
        template.summaryMetrics.forEach(metric => {
          emptySummaryTemplate[metric] = {};
          emptySummaryTemplate[metric]['activeGraphs'] =
            template.data.SUMMARY.graphTemplate.activeGraphs;
          emptySummaryTemplate[metric] =
            template.data.SUMMARY.INSTANCE_GLOBAL[metric];
        });
      }
    }
  ...
  toggleActiveMetric(category, metric, graph) {
    this[category][metric].activeGraphs[graph] = !this[category][metric]
      .activeGraphs[graph];
  }
      ...
      if (categoryTracker === 'RENDER' && i < 2) {
        Object.keys(this[categoryTracker]).forEach(metric => {
          if (this.RENDER[metric].activeGraphs[i]) {
            this.RENDER[metric].isRenderedOnGraph[i]
              ? (this.RENDER[metric].animationIsActive = false)
